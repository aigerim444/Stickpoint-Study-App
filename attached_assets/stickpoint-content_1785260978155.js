...
// Offline fallback card builder, only used when the developer explicitly enables
// "forceOfflineGrading", never as a silent fallback on API failure. Produces the
// same {question, answer, methodTag} shape the API returns.
export function localExtractCards(text, count = 8) {
...
export function localTopic(text) {
  const first = cleanSentences(text)[0] || "";
  const words = first.split(" ").slice(0, 6).join(" ");
  return words || "Your material";
}

// Validates the shape the API (or the offline fallback) must produce.
export function isValidCards(cards) {
  return Array.isArray(cards) && cards.length > 0 && cards.every(c =>
    c && typeof c.question === "string" && c.question.trim().length > 0 &&
    typeof c.answer === "string" && c.answer.trim().length > 0
  );
}

// Cards already arrive flashcard-shaped {question, answer, methodTag} from the API -
// this just guards against missing fields so the UI never renders undefined.
...
const stripChoiceLetter = (t) => String(t || "").replace(/^\s*[A-Da-d][.)]\s+/, "").trim();

export function buildFlashcards(cards) {
  return cards.map((c) => ({
    question: c.question,
...
export function buildMCQ(cards) {
...
export function keywordOverlapFeedback(cards, userText) {
...
export const SUBJECT_METHOD_MAP = {
...
// Q1 is the subject question, handled separately. Q2-Q7 below score methods directly.
...
// Chop the pixel mascot, one bitmap, moods are done with CSS transforms/filters around it.
...
export function pipGrid(palette) {
  const colors = Object.assign({
    ".": "transparent", K: "#1a1a2e", B: "#ff6b4a", E: "#ffffff"
  }, palette || {});
  return PIP_BITMAP.map(row => row.split("").map(ch => colors[ch] || "transparent"));
}

export function scoreQuiz(subjectId, answerIdxArr) {
  const scores = {};
  METHODS.forEach(m => scores[m.id] = 0);
  QUIZ_QUESTIONS.forEach((question, qi) => {
    const raw = answerIdxArr[qi];
    if (raw == null) return;
    // An answer is either one option index, or an array of up to two indices when the
    // student was torn. Tied options split the question's weight evenly between them,
    // whether or not they sit next to each other in the list.
    const picked = (Array.isArray(raw) ? raw : [raw]).filter(i => i != null);
    if (!picked.length) return;
    const weight = 1 / picked.length;
    picked.map(i => [question.options[i], weight]).forEach(([opt, weight]) => {
      if (!opt) return;
      Object.entries(opt.pts).forEach(([id, pts]) => { scores[id] = (scores[id] || 0) + pts * weight; });
    });
  });
  ...
  // Free-recall prose methods don't transfer to procedural math, keep them out of the top 3.
  if (subjectId === 'math_science') ['blurting', 'self_explanation'].forEach(id => scores[id] = (scores[id] || 0) - 4);
  // Problem Sets only makes sense where there are problems to work.

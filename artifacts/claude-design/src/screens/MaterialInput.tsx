...
export function MaterialInputScreen({ v }: any) {
  ...
  if (v.pdfLoading || v.photoLoading) {
    const label = v.pdfLoading ? 'Reading your PDF…' : 'Reading your photo…';
    const sub = v.pdfLoading
      ? 'Extracting text — hang tight, this takes a few seconds.'
      : 'Pulling text from your image — almost there.';
    return (
      <div style={css('flex:1;min-height:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:22px;padding:32px;')}>
        <div style={{ fontSize: 44 }}>📄</div>
        <div style={css("font-family:'Press Start 2P';font-size:11px;color:#201E2E;text-align:center;line-height:1.8;")}>{label}</div>
        <div style={css('font-size:13px;font-weight:700;color:#8a8194;text-align:center;max-width:260px;line-height:1.6;')}>{sub}</div>
        {/* animated progress bar */}
        <div style={css('width:100%;max-width:280px;height:10px;background:#F1E4CC;border:2px solid #201E2E;overflow:hidden;')}>
          <div style={{
            height: '100%',
            width: '45%',
            background: '#FF6B4A',
            borderRight: '2px solid #201E2E',
            animation: 'pdfSlide 1.4s ease-in-out infinite',
          }} />
        </div>
        <style>{`
          @keyframes pdfSlide {
        ...
        `}</style>
        <div style={css('font-size:11px;font-weight:700;color:#c9c2b8;')}>Chop is on it…</div>
      </div>
    );
  }
    ...
    <div data-screen-label="Material Input" style={css('flex:1;min-height:0;overflow-y:auto;display:flex;flex-direction:column;padding:clamp(18px,3vh,28px) 24px;gap:14px;animation:fadeUp .4s ease both;')}>
      ...
      {v.isMaterialModePdf && (
        <div style={css('display:flex;flex-direction:column;gap:10px;')}>
          ...
          {v.pdfStatusText !== 'No PDF chosen yet' && (
      ...
      {(v.isMaterialModePdf || v.isMaterialModePhoto) && v.materialDraft && (
        <button onClick={v.submitMaterial} disabled={v.materialDisabled} style={{ fontFamily: 'Nunito', fontWeight: 900, fontSize: 15, color: '#fff', background: v.materialBtnColor, border: '3px solid #201E2E', boxShadow: '4px 4px 0 #201E2E', padding: 15, cursor: v.materialBtnCursor, marginTop: 4 }}>EXTRACT &amp; STUDY →</button>
      )}
    </div>
  );
